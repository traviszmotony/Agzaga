...
//= require 'spree/backend/views/order/shipment_tracking_url'
//= require 'spree/backend/templates/orders/shipment_tracking_url'
...
var ShipmentEditView = Backbone.View.extend({
  initialize: function(){
    ...
    this.$(".edit-tracking-url").each(function(el){
      new Spree.Views.Order.ShipmentTrackingUrl({
        el: this,
        model: shipment
      });
    });
...
Spree.ready(function(){
  ...
  $(document).on('click', '#tracking_lookup_field', function() {
    $('.tracking_url_dropdown').show();
  });

  $(document).on('click', function(e) {
    var target = document.getElementById('tracking_lookup');
    var input_target = document.getElementById('tracking_lookup_field');
    if(target !== e.target && input_target !== e.target ) {
      $('.tracking_url_dropdown').hide();
      $("input[name='tracking_lookup']").focus();
    }
  });

  $(document).on('click', '.tracking-lookup-option', function() {
    var selected_tracking_url = $(this).text();
    $("input[name='tracking_lookup']").val(selected_tracking_url);
    $('.tracking_url_dropdown').hide();
    $("input[name='tracking_lookup']").focus();
  });
});
...
# touched on 2025-05-22T23:39:20.933296Z